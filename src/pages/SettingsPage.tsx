...
export function SettingsPage() {
  const { settings, resolvedTimezone, setTimezone, setAiInstruction, setAnimation } = useSettings()

  return (
    <div className="pt-6">
      <p className="mb-1 text-[10px] tracking-[0.24em] text-fog">系统设置</p>
      <h1 className="mb-6 text-lg font-bold tracking-[0.2em]">参数配置</h1>
      ...
      <section className="panel p-4 sm:p-5">
        ...
        <select
          ...
          aria-label="时区"
          className="w-full max-w-md border border-edge bg-void px-3 py-2 text-[12px] text-ink focus:border-signal focus:outline-none"
        >
      ...
      <section className="panel mt-4 p-4 sm:p-5">
        ...
        <dl className="grid grid-cols-1 gap-y-2 text-[11px] sm:grid-cols-2">
          <Info label="位序" value="初爻 = BIT 0（固定）" />
          <Info label="引擎" value={RULE_VERSION} />
          <Info label="数字算法" value={NUMBER_METHOD_VERSION} />
          <Info label="时间算法" value={TIME_METHOD_VERSION} />
...
function ToggleBtn({ active, onClick, children }: { active: boolean; onClick: () => void; children: string }) {
  return (
    <button
      ...
      onClick={onClick}
      aria-pressed={active}
      className={`border px-4 py-2 text-[11px] tracking-[0.16em] ${
